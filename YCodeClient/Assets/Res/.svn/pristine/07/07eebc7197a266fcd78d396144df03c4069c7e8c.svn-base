Shader "Hidden/Terrain/Previewer" { 
	Properties {
			...
			CGPROGRAM
			   ...
			   v2f vert(appdata_full v){
				...
				half4 frag (v2f i) : SV_Target{
					float2 uv2 =float2(i.uv2.x*i.uv1.x , i.uv2.y *i.uv1.y);
        			float2 uv = float2(i.uv2.x *_Tiling.x,i.uv2.y *_Tiling.y);
				 
        			half4 col =tex2D(_MainTex, UNITY_PROJ_COORD(uv));  
        			half4 mask =tex2Dproj (_MaskTex,UNITY_PROJ_COORD(i.uv2));
					col.a=1;
				//	mask.a=1; // the alpha channel of the paiting material some how used as specular data, so we put it to 1 as preview; 
