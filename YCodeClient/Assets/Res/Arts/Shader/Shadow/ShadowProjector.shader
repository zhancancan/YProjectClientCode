	...
	SubShader {
		...
		Pass {
			...
			float4 frag(v2f i):COLOR{				
				...
				//a += tex2D(_ShadowTex, uv + _ShadowTex_TexelSize.xy * _BlurWidth * float2(1,0)).a;
				//a += tex2D(_ShadowTex, uv + _ShadowTex_TexelSize.xy * _BlurWidth * float2(0,1)).a;
				//a += tex2D(_ShadowTex, uv + _ShadowTex_TexelSize.xy * _BlurWidth * float2(-1,0)).a;
				//a += tex2D(_ShadowTex, uv + _ShadowTex_TexelSize.xy * _BlurWidth * float2(0,-1)).a;
				//a = a/5;
